...
Possible random effects: paper ID, cohort_ID, strain, lineage, shared control, age

```{r}
meta1null <- rma.mv(yi, vi, data=Effect_Size, method = 'ML') 
metastudy <- rma.mv(yi, vi,random = ~1|Cohort_ID, data=Effect_Size, method = 'ML') 
meta1study <- rma.mv(yi, vi,random = ~1|Age_Days, data=Effect_Size, method = 'ML') 
meta2study <- rma.mv(yi, vi,random = ~1|Rodent_type, data=Effect_Size, method = 'ML') 
meta3study <- rma.mv(yi, vi,random = ~1|Lineage_HFD, data=Effect_Size, method = 'ML') 
meta4study <- rma.mv(yi, vi,random = ~1|Shared_Control, data=Effect_Size, method = 'ML') 
meta5study <- rma.mv(yi, vi,random = ~1|Strain, data=Effect_Size, method = 'ML') 

metastudy_best <- rma.mv(yi, vi,random = list(~1|Paper_ID,~1|Cohort_ID,~1|Age_Days), data=Effect_Size, method = 'ML') 
```
...
```{r, echo=FALSE}
anova(meta1null, meta6study)
```

Results:
I had a look at different random effects and compiled a model that I think is best. Paper ID, Cohort ID, and Age seem to yiled the lowest AIC values. The rest don't seem to be that important. 
...
```
# **3. Subsetting Data into multigenerational and one off exposure**
```{r, echo=FALSE}
